...
class Patient < ApplicationRecord
        ...
        ->(organisation:) do
          joins(
            "LEFT JOIN archive_reasons " \
              ...
              "AND archive_reasons.organisation_id = #{organisation.id}"
          )
        end

  scope :archived,
        ->(organisation:) do
          joins_archive_reasons(organisation:).where(
  ...
  def archived?(organisation:)
    archive_reasons.exists?(organisation:)
  end

  def not_archived?(organisation:)
    !archive_reasons.exists?(organisation:)
  end
